...
{
    public class ReadRepositoryUser<T> : IReadRepositoryUser<T> where T : User
    {
        private readonly MichalBialekDbContext _context;

        public ReadRepositoryUser(MichalBialekDbContext context)
        {
            _context = context;
        }
        ...
        public IList<T> GetByLoginAndPassword(string login , string password)
        {
            return _context.Set<T>().Where(x => x.Login == login && x.Password == password).ToList();
        }

        public IList<T> GetByPesel(int pesel)
        {
